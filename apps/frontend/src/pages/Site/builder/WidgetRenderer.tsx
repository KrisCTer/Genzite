...
const WidgetRenderer: React.FC<WidgetRendererProps> = ({ type, config = {}, isActive }) => {
  ...
  const scale = config.geometry?.scale || config.scale || 1;
        ...
        <div style={getStyle('transparent', '24px')}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px', marginTop: '24px' }}>
            {(config.items || [1, 2, 3]).map((_item: any, i: number) => (
              <div key={i} style={{ height: '150px', background: 'var(--gz-dark-4)', borderRadius: 'var(--radius-md)', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }}>
                {_item && (_item.image || typeof _item === 'string') ? (
                  <img src={typeof _item === 'string' ? _item : _item.image} alt="gallery" style={{ width: '100%', height: '100%', objectFit: config.imageFit || 'fill' }} />
                ) : (
      ...
      return (
        <div style={getStyle('transparent', '24px')}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(250px, 1fr))', gap: '24px' }}>
            {(config.products || []).map((product: { id?: string; name?: string; price?: number | string; image?: string }, i: number) => (
              <Card 
                key={product.id || i} 
                hoverable
                variant="borderless" 
                ...
                cover={
                  <div style={{ height: '200px', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--gz-dark-2)' }}>
                    {product.image ? (
                      <img src={product.image} alt={product.name} style={{ width: '100%', height: '100%', objectFit: config.imageFit || 'fill' }} />
                    ) : (
              ...
              >
                ...
                <Title level={4} style={{ color: 'var(--color-accent)', margin: '0 0 16px 0' }}>${Number(product.price || 0).toLocaleString()}</Title>
                <Button 
                  type="primary" 
                  block 
                  ...
                  onClick={() => {
                    ...
                    messageApi.success(`Added ${product.name || 'item'} to cart!`);
                  }}
                >
                  Add to Cart
                </Button>
