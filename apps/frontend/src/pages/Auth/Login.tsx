...
import { signIn, signOut, fetchAuthSession, signUp, signInWithRedirect } from 'aws-amplify/auth';
...
const Login: React.FC = () => {
    ...
    onSuccess: (data) => {
      ...
      if (data.isCognito) {
        if (data.nextStep?.signUpStep === 'CONFIRM_SIGN_UP') {
          message.info('A verification link has been sent to your email. Please click it to verify your account.');
          setIsSignUp(false);
  ...
  const handleSocialLogin = (platform: 'Google' | 'GitHub'): void => {
    ...
    if (cognitoUserPoolId && cognitoClientId && !cognitoUserPoolId.includes('xxxxxx')) {
      try {
        signInWithRedirect({
          provider: platform === 'Google' ? 'Google' : { custom: platform }
        });
      } catch (err) {
        console.error(`Error signing in with ${platform}`, err);
        message.error(`Failed to initiate login with ${platform}.`);
      }
    } else {
      message.info(`OAuth login via ${platform} is not available in local mode.`);
    }
  };
                ...
                >
                  ...
                  <div className="flex justify-center gap-3">
                    <Button
                      ...
                      className="bg-white/5 border border-white/10 hover:border-white/20 hover:bg-white/10 text-white flex-1 h-10 flex items-center justify-center transition-colors"
                    >
                      Google
                    </Button>
                    <Button
