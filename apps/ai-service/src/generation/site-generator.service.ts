  ...
  private async generateHybrid(
    ...
    try {
      ...
      const runLlmWorker = async (sec: any) => {
        ...
        try {
          ...
          return {
            ...
            contentConfig: widgetResult?.contentConfig || { title: sec.type }
          };
        } catch (e) {
          ...
          try {
            ...
            return {
              type: sec.type,
              sortOrder: sec.sortOrder,
              contentConfig: fallbackResult?.contentConfig || { title: sec.type }
            };
        ...
        if (sec.assignTo === 'stitch' || sec.type === 'HEADER' || sec.type === 'HERO') {
          try {
            ...
            return {
              type: sec.type,
              sortOrder: sec.sortOrder,
              contentConfig: extractionResult?.contentConfig || { title: sec.type }
            };
          } catch (e) {
      ...
      const result: GeneratedSite = {
        ...
        site: { name: pageTitle, subdomain: generatedSubdomain },
        pages: [{
          ...(targetPageId ? { id: targetPageId } : {}),
          title: pageTitle,
          slug: pageSlug,
